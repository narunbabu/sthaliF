...
var UserService = (function () {
    ...
    UserService.prototype.create = function (user) {
        return this.http.post(this.config.apiUrl + '/auth/register', user, this.jwt()).map(function (response) {
    ...
    UserService.prototype.getUserVillage = function () {
        var currentUser = JSON.parse(localStorage.getItem('currentUser'));
        // console.log(this.http.get(this.config.apiUrl + '/uv', this.jwt()).map((response: Response) => response.json()))
        // return this.jwt();
        return this.http.get(this.config.apiUrl + '/uv?token=' + currentUser.token).map(function (response) { return response.json(); });
    };
